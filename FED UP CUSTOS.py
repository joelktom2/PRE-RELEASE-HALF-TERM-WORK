...
def ChangeSettings():
  ...
  finput = input("do you want customers to get fed up ? Y/N: ")
  if finput == "Y":
    fut = int(input("How long do you wish customers to wait before they get fed up"))
  else:
    fut = -1
  
  return SimulationTime, NoOfTills,fut
...
def IncrementTimeWaiting(BuyerQ, QLength,fut):
  lost_custos = 0
  for Count in range(QLength):
    BuyerQ[Count].WaitingTime += 1
    if fut != -1 and BuyerQ[Count].WatingTime > fut:
      BuyerQ.remove(BuyerQ[Count])
      lost_custos += 1
  return BuyerQ,lost_custos
...
def Serving(Tills, NoOfTills, BuyerQ, QLength, Stats,fut):
  ...
  while TillFree != -1 and QLength > 0:
    ...
    TillFree = FindFreeTill(Tills, NoOfTills)
  BuyerQ,lost_custos = IncrementTimeWaiting(BuyerQ, QLength,fut)
...
def QueueSimulator():
  ...
  SimulationTime, NoOfTills,fut = ChangeSettings()
  ...
  for TimeUnit in range(SimulationTime):
    ...
    if TimeToNextArrival == 0:
      ...
      print()
    Tills, NoOfTills, BuyerQ, QLength, Stats = Serving(Tills, NoOfTills, BuyerQ, QLength, Stats,fut)
  ExtraTime = 0
  while QLength > 0:
